# RR-Model (Resolution/Reconstruction) Training Script - Complete Kaggle Notebook
...
print("🚀 Starting RR-Model (Resolution/Reconstruction) Training...")
print("📁 Data directory: /kaggle/input/dfdc-10")
print("💾 Output directory: /kaggle/working")
print("🎯 Model type: RR (Resolution Analysis)")
print("⏱️  Expected training time: 13-15 hours")
...
class RRSpecialistModel(nn.Module):
    """RR Specialist model with transfer learning"""
...
class RRDataset(Dataset):
    """Dataset for RR model with directory-based structure"""
    ...
    def _extract_frames(self, video_path, num_frames=8):
        ...
        try:
            ...
            if total_frames > 0:
                # Select frames strategically for resolution analysis
...
class RRTrainer:
    """Trainer for RR specialist model"""
    def __init__(self, data_dir, output_dir):
        ...
        # Initialize model
        self.model = RRSpecialistModel(num_classes=2)
    ...
    def _load_latest_checkpoint(self):
        ...
        checkpoint_pattern = "rr_model_*.pt"
    ...
    def _save_checkpoint(self, epoch, metrics):
        ...
        checkpoint_path = self.output_dir / f"rr_model_epoch_{epoch:03d}.pt"
        ...
        checkpoints = sorted(self.output_dir.glob("rr_model_epoch_*.pt"))
    ...
    def train(self, num_epochs=20):
        """Train the RR model"""
        logger.info("Starting RR specialist model training")
        ...
        dataset = RRDataset(self.data_dir, transform)
        ...
        for epoch in range(num_epochs):
            ...
            for batch_idx, (frames, labels) in enumerate(dataloader):
                ...
                # Log progress
                if batch_idx % 50 == 0:
                    logger.info(f"RR - Epoch {epoch+1}/{num_epochs}, Batch {batch_idx}, Loss: {loss.item():.4f}")
            ...
            logger.info(f"RR - Epoch {epoch+1}/{num_epochs} completed. "
        ...
        # Save final model
        final_model_path = self.output_dir / "rr_model_final.pt"
        torch.save({
            'model_state_dict': self.model.state_dict(),
            'training_history': self.training_history,
            'model_type': 'rr',
        ...
        logger.info(f"Final RR model saved: {final_model_path}")
        
        # Save training history
        history_path = self.output_dir / "rr_training_history.json"
        with open(history_path, 'w') as f:
            json.dump(self.training_history, f, indent=2)

# Initialize and start training
trainer = RRTrainer('/kaggle/input/dfdc-10', '/kaggle/working')
trainer.train()

print("✅ RR-Model training completed!")