...
class OptimizedSpecialistDataset(Dataset):
    ...
    def __init__(self, data_dir, chunk_idx, model_type='ll', transform=None):
        ...
        print(f"📂 Loading data from chunk {chunk_idx:02d}...")
        self._load_chunk_data()
        print(f"✅ Loaded {len(self.samples)} samples from chunk {chunk_idx:02d}")
    ...
    def _load_chunk_data(self):
        ...
        if not all_dirs:
            print("❌ No directories found in dataset!")
            return
        ...
        for pattern in chunk_patterns:
            for dir_name in all_dirs:
                if pattern in dir_name.lower():
                    ...
                    print(f"🎯 Found potential chunk directory: {dir_name}")
                    ...
                    if video_count > 0:
                        ...
                        print(f"✅ Using directory with {video_count} video files: {chunk_path}")
                        break
                    elif subdirs:
                        ...
                        for subdir in subdirs:
                            ...
                            try:
                                ...
                                if sub_video_count > 0:
                                    ...
                                    print(f"✅ Using subdirectory with {sub_video_count} video files: {chunk_path}")
                                    break
                            except:
                                continue
        ...
        if chunk_path is None:
            print(f"❌ Could not find chunk {self.chunk_idx} in any of these directories:")
            for dir_name in all_dirs:
                print(f"   📁 {dir_name}")
            print("💡 Please check the dataset structure and update the script accordingly.")
            return
        
        print(f"📂 Scanning folder: {chunk_path}")
        ...
        if metadata_file.exists():
            try:
                ...
                print("⚠️ Could not load metadata, using filename-based labeling")
...
class OptimizedSpecialistTrainer:
    ...
    def _load_latest_checkpoint(self):
        ...
        if checkpoints:
            ...
            print(f"✅ Resumed from chunk {self.current_chunk}")
    ...
    def train_on_chunk(self, chunk_idx):
        ...
        print(f"\n🎯 Starting {self.model_type.upper()} specialist training on chunk {chunk_idx}")
        print("="*60)
        ...
        if len(dataset) == 0:
            print(f"⚠️ No data found in chunk {chunk_idx}")
            return
        ...
        for epoch in range(4):  # 4 epochs per chunk for specialist models
            print(f"\n📈 Epoch {epoch+1}/4 for chunk {chunk_idx}")
        ...
        print(f"📊 Final metrics: Accuracy={accuracy*100:.2f}%, Loss={avg_loss:.4f}")
        print("="*60)
    ...
    def train_all_chunks(self):
        ...
        print(f"\n🚀 Starting {self.model_type.upper()} specialist model training on ALL 10 chunks")
        print(f"🎯 Target: Process 100GB of data with full utilization")
        print("="*80)
        ...
        for chunk_idx in range(self.current_chunk, self.total_chunks):
            try:
                ...
                print(f"\n📈 OVERALL PROGRESS: {progress:.1f}% ({chunk_idx+1}/10 chunks)")
                print(f"⏱️  Chunk {chunk_idx} took: {time_taken/3600:.2f} hours")
        ...
        print(f"📊 Total chunks processed: {self.current_chunk}/10")
        print("="*80)
...
def main():
    ...
    print("🚀 KAGGLE T4x2 OPTIMIZED SPECIALIST TRAINING STARTING...")
    ...
    print("🎉 SPECIALIST TRAINING COMPLETED SUCCESSFULLY!")
