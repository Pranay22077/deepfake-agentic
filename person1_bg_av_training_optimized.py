...
class OptimizedDataset(Dataset):
    ...
    def __init__(self, data_dir, chunk_file, model_type='bg', transform=None):
        ...
        print(f"📂 Loading data from {chunk_file}...")
        self._load_chunk_data()
        print(f"✅ Loaded {len(self.samples)} samples from {chunk_file}")
...
class OptimizedTrainer:
    ...
    def __init__(self, model_type, data_dir, output_dir):
        ...
        print(f"🚀 Initializing {model_type.upper()} trainer for Kaggle T4x2...")
        ...
        print(f"✅ {model_type.upper()} trainer ready!")
    ...
    def train_on_chunk(self, chunk_idx):
        ...
        print(f"\n🎯 Starting {self.model_type.upper()} training on chunk {chunk_idx}: {chunk_file}")
        print("="*60)
        ...
        if len(dataset) == 0:
            print(f"⚠️ No data found in chunk {chunk_file}")
            return
        ...
        for epoch in range(3):  # 3 epochs per chunk
            print(f"\n📈 Epoch {epoch+1}/3 for chunk {chunk_idx}")
    ...
    def train_all_chunks(self):
        ...
        print(f"\n🚀 Starting {self.model_type.upper()} model training on ALL 10 chunks")
...
def main():
    ...
    print("🚀 KAGGLE T4x2 OPTIMIZED TRAINING STARTING...")
