...
\begin{tikzpicture}[scale=0.8, transform shape]
    ...
    % Responsibilities
    \node[process, fill=convColor!20] (perception) at (-5,3) {🔍 PERCEPTION\\Extract metadata};
    \node[process, fill=bnColor!20] (reasoning) at (0,4.5) {🧠 REASONING\\Evaluate confidence};
    \node[process, fill=reluColor!20] (planning) at (5,3) {📋 PLANNING\\Select models};
    \node[process, fill=poolColor!20] (action) at (5,-3) {⚡ ACTION\\Execute inference};
    \node[process, fill=fcColor!20] (learning) at (0,-4.5) {📚 LEARNING\\Incorporate feedback};
    \node[process, fill=lstmColor!20] (explanation) at (-5,-3) {💬 EXPLANATION\\Generate justifications};
