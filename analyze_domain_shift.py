...
class DomainAnalyzer:
    ...
    def analyze_test_dataset(self):
        ...
        print("🔍 Analyzing Test Dataset Characteristics")
        print("=" * 60)
    ...
    def compare_with_dfdc_characteristics(self, real_stats, fake_stats):
        ...
        if '224' in common_res or '256' in common_res:
            print("  ✅ Resolution: Similar to DFDC")
        else:
            print(f"  ⚠️  Resolution: Different from DFDC ({common_res})")
        
        # FPS analysis
        if 25 <= avg_fps <= 35:
            print("  ✅ FPS: Similar to DFDC")
        else:
            print(f"  ⚠️  FPS: Different from DFDC ({avg_fps:.1f})")
        
        # Duration analysis
        if 5 <= avg_duration <= 15:
            print("  ✅ Duration: Similar to DFDC")
        else:
            print(f"  ⚠️  Duration: Different from DFDC ({avg_duration:.1f}s)")
        
        # Quality analysis
        if avg_sharpness < 100:
            print("  ⚠️  Quality: Lower than typical DFDC")
        elif avg_sharpness > 500:
            print("  ⚠️  Quality: Higher than typical DFDC")
        else:
            print("  ✅ Quality: Similar to DFDC")
...
def main():
    ...
    print("🔍 E-Raksha Domain Shift Analysis")
    ...
    if analysis:
        ...
        print(f"\n🎯 DOMAIN SHIFT SCORE: {shift_score:.2f}")
        
        if shift_score < 0.3:
            print("✅ LOW domain shift - Model should work well")
        elif shift_score < 0.6:
            print("⚠️  MODERATE domain shift - 45% accuracy expected")
        else:
            print("❌ HIGH domain shift - Poor performance expected")
